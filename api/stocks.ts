...
// Comprehensive Bourse de Casablanca (BVC) listed shares dictionary
const ALL_CASABLANCA_STOCKS: Record<string, { symbol: string; companyName: string; price: number; changePercent: number; sector: string }> = {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      source: 'Casablanca Stock Exchange (Bourse de Casablanca - BVC) Full Market Feed',
      timestamp,
      totalStocks: Object.keys(ALL_CASABLANCA_STOCKS).length,
      stocks: ALL_CASABLANCA_STOCKS
    });
  } catch (error: any) {
    return res.status(500).json({
      success: false,
      error: error?.message || 'Failed to fetch Casablanca Bourse share prices',
      stocks: ALL_CASABLANCA_STOCKS
    });
  }
}
